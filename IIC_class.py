...
def main(config):
    ...
    o_c = config.TRAIN.OVER_CLUSTERING

    print('experiment no. ---->', no)
    print('data_type ---->', data_type)
    print('over_cluestering ---->', o_c)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for batch_data in tqdm(origin_loader):
            ...
            if o_c == False:
                x1_out, x1_over = net(x_1)
                x2_out, x2_over = net(x_2)
                ...
                x1_out, x1_over_out = net(x_1)
                x2_out, x2_over_out = net(x_2)

                loss1 = IID_loss(x1_out, x2_out)
                loss2 = IID_loss(x1_over_out, x2_over_out)
                loss = loss1 + loss2
    ...
    # for i in loss_List:
    make_graph('Epochs', 'Loss', loss_List, train_loss_value, net_path, no)
